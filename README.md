...
- __eventname__: the event to listen for _(String)_
  ...
  - __message__: event fires when the `InAppBrowser` receives a message posted from the page loaded inside the `InAppBrowser` Webview.
    - __customscheme__: event fires when the browser navigates to a URL using one of the schemes listed in the `AllowedSchemes` preference.
    - __download__: (**Android** only) event fires when navigating to a URL results in a file download request.
...
function showHelp(url) {
    ...
    inAppBrowserRef.addEventListener('download', downloadCallback);
}
...
function downloadCallback(params) {
    console.log('Download requested', params.url, params.mimetype);
}
...
- __type__: the eventname, either `loadstart`, `loadstop`, `loaderror`, `message`, `customscheme`, `download` or `exit`. _(String)_

- __url__: the URL that triggered the event. _(String)_
...
- __userAgent__: user agent string received with the download request. _(String, Android download event only)_

- __contentDisposition__: value of the `Content-Disposition` header associated with the download, if present. _(String, Android download event only)_

- __contentLength__: file size reported by the download request. _(Number, Android download event only)_

- __mimetype__: MIME type reported by the download request. _(String, Android download event only)_
