...
type channel = "loadstart" | "loadstop" | "loaderror" | "exit" | "message" | "beforeload" | "customscheme" | "download";
...
interface InAppBrowserEvent extends Event {
    ...
    message?: string;
    /** the message contents , only in the case of `message`. */
    data?: string;
    /** the mimetype reported by the download event (Android only). */
    mimetype?: string;
    /** the reported user agent when the download event fired (Android only). */
    userAgent?: string;
    /** Content-Disposition header provided with the download request (Android only). */
    contentDisposition?: string;
    /** File size advertised by the download request (Android only). */
